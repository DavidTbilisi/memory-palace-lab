...
/** Past this many stops the method suggests splitting a route into stretches. */
const LONG_ROUTE_STOPS = 25;
  ...
  return (
    <ol aria-label="Stops" className="mt-2 space-y-px">
      {stops.map((stop, index) => {
        ...
        const due = countDueLoci([stop], nowIso) > 0;
        return (
          <li
            key={stop.id}
            ref={(element) => {
...
function RouteCard({
  ...
  const dueCount = countDueLoci(stops);
  const startRename = () => {
        ...
        <div className="border-t border-zinc-800 px-2 pb-2 pt-1.5">
          ...
          {stops.length > LONG_ROUTE_STOPS ? (
            <p className="mt-2 text-[11px] leading-4 text-zinc-500">
              Long routes are easier to rehearse in stretches of 12 to 25 stops.
            </p>
          ) : null}
        </div>
      ) : null}
