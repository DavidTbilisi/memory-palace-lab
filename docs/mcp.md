...
- **Nodes** — `node_list`, `node_get`, `node_create`, `node_update`, `node_delete`. `node_create` and `node_update` take `nedf` with any of `nameHook`, `essence`, `distinguisher: {prompt, reason}` and `failure: {scenario, correction}`. An update changes only the slots it names: `null` or `""` clears one slot, and `nedf: null` clears all four. `node_get` returns the slots, the ones still missing (`unencodedSlots`), and each stop's schedule per slot. `route_list` shows the same per-slot schedules on each stop.
