...
**Last updated**: 2026-09-18 (evening — §Stage 4 gained its **first readings**: two `cast::pattern_flash` sessions, 75 %/26.9 s then 100 %/8.9 s, sketch form 4.4 s; run-record at 2026-09-18-cast-pattern-flash-first-readings); 2026-09-18 (§Stage 4 gained **both runners** — `tools/cast_pattern_flash.py` and `tools/cast_tier2_flash.py`, one event per rep each, over a shared core, with `--report` deriving the band; and the note that four of the eight cues need [edge-sign](./edge-sign.md) marks to be separable by sketch at all); 2026-08-20 (§Visual authored — diagram replaces the TODO stub); 2026-08-20 (§Checksum authored — 3 falsifiable retrieval questions replace the TODO stub); 2026-08-20 (§Mnemonic authored — TODO stub replaced with a real device); 2026-08-20 (`glyph:` assigned — [representation-rules](./representation-rules.md) Rule 11); 2026-07-09 — [edge-sign](./edge-sign.md) 16→18 pool note added (gated on its promotion); 2026-07-06 — Stage 4 gained the two measured automaticity flashes (`cast::pattern_flash`, `cast::tier2_vocab_flash`) with METER floors; 2026-05-02 original.
...
**Runner (`cast::pattern_flash`, wired 2026-09-18)**: `python3 tools/cast_pattern_flash.py` — 16 reps by default (two round-robin passes, so no cue can go unseen), ASCII topology or one-line structure, unique-prefix answers, one event per rep. `--report` reads the series back and names the band; `--no-emit` practises without writing. The session summary is **derived from the events, never stored** — the floor is recomputed from the log, so a band cannot drift from the reps that produced it.

**Runner (`cast::tier2_vocab_flash`, wired 2026-09-18)**: `python3 tools/cast_tier2_flash.py` — 32 reps by default, every one of the 16 items once in each direction. **Forward** (slot + meaning → item) is free recall answered with the item's two-letter [edge code](./nodes-and-edges.md), whose registered scope already includes drill answers — two keystrokes keep typing cost out of a 2-second floor without putting a menu on screen. **Reverse** (item → meaning) is a four-way choice among the *same slot's* meanings, shuffled per rep: recognition rather than recall, accepted deliberately because a typed meaning would measure the keyboard. The pool is pinned at 16 by a test, per the edge-sign rule below, and a second test checks the pool against the code table on [nodes-and-edges](./nodes-and-edges.md) so the drill cannot drift from its owner. `--report` adds a **confusion table** (expected → answered) — the instrument for the `Me`/`Ma` discrimination pair.

**Whole-encode logger (wired 2026-09-18)**: `python3 tools/cast_encode_log.py {step0|encode|walk|report} <graph>` over a graph file in `tools/cast-graphs/`. `step0` computes the four checks of [step-zero-analysis](./step-zero-analysis.md) plus the pattern scan, so the operator *verifies* the analysis instead of performing it; `encode` puts a clock on every node and every edge (bare Enter = Tier 1, unmarked — marks are opt-in per edge); `walk`, run a day later, scores each bundle from memory, re-derives each loop's sign, and reads back only the quantities that were marked. It emits the real-encode events under the names their owner pages give them (`cast.scene_count`, `cast.out_degree_max`, `cast.generator_test_run`, `cast::edge_sign_used`, `cast::loop_sign_check`, `cast.quantity_tier_used`, `cast.quantity_recall`, `cast.delay_move_used`), and gives event names to two drills this stage lists without one: **`cast.encode_seconds`** (the *3-minute animal assignment* and *5-minute edge-bundle generation* drills, per node and per edge) and **`cast.walk_recall`** (the *timed palace walk from hub outward*). **Neither has a floor yet** — there are no readings to set one from, and a floor invented before the first measurement is a guess wearing a number. First graph: `walker-sleep` (20 nodes, 27 edges, one leash, one spiral), every edge cited to the wiki page that carries it.

**First readings, 2026-09-18** — 2026-09-18-cast-pattern-flash-first-readings. Two `cast::pattern_flash` sessions: cold 75 % / 26.9 s (fallback rule fired), then 100 % / 8.9 s after five untimed minutes on the plate, with the **sketch form alone at 4.4 s / 100 %**. Neither passes floor yet. Two findings feed back here: the **one-line form is a second skill**, not a second presentation of the first (35.6 → 11.0 s against sketch's 10.8 → 4.4 s), so it is layered in only after sketches pass floor; and **every error was a mark-separated pair** — spiral/leash, chain/dominoes — which is where [edge-sign](./edge-sign.md)'s notation is actually load-bearing for a learner rather than merely for the deck.

Both runners share one core (`tools/cast_flash_core.py`) for timing, scoring, emission and reporting, so *"both flashes at floor"* in the pass rule below means one thing.

Two of the eight cues are not separable by topology alone — `chain` and `dominoes` are the same digraph, and `spiral` and `leash` the same cycle — so the sketch form draws the marks CAST itself uses: `(X)` on cascade nodes, `(+)`/`(-)` edge signs on the loops. That makes this drill the first operational dependent of [edge-sign](./edge-sign.md), which is still a 🟡 candidate.
