...
**Last updated**: 2026-09-18 — §Edge codes: the `Me`/`Ma` failure mode gained its instrument (`tools/cast_tier2_flash.py --report` confusion table); §Related Pages gained [adjacency-is-not-an-edge](./adjacency-is-not-an-edge.md). 2026-09-05 — §Modifier composition: the lattice **named as GoF Decorator** (adopted at [software-design-principles-for-neural-os](./software-design-principles-for-neural-os.md) §GoF Decorator), which supplies the combinatorial cost of refusing additivity and the reason there is no recursion here. §Edge codes: **scope claim corrected** from *serialize an edge* to *serialize a Tier 2 scene*, and the gap named — quantity and delay have no written form, so *thick stream, long gap, interceptor* serializes identically to a thin, instant edge. Notation left to the operator; one derived constraint (self-delimiting) recorded. 2026-09-05 — §Modifier composition: **loop gain** named as a second open gap, surfaced by filing Meadows' leverage rungs against the encoder (rungs 8 and 7 are gain rungs; CAST owns polarity and not magnitude around a cycle). 2026-09-04 — §Layer 1: break-the-isomorphism rule added, from st-example-ecological-system (an encoding that mirrors its subject has no error detection). 2026-09-04 (ingest-ghost pass: tier-1/tier-2 self-links de-linked to their own sections, `major-system`/`pao`/`names-and-faces`/`loci` repointed to real owners, `feedback-loops` → feedback-loop-taxonomy, web-service example → [cast-example-city-streets](./cast-example-city-streets.md)); 2026-09-04 — §Modifier composition added: the three additive modifiers stated as a lattice (disjoint positions, no order), the "cheapest mark that discriminates" rule promoted to cover all three, depth-as-a-nested-ladder rejected via `/validate-idea`, and edge confidence named as the open gap; 2026-08-31 — §Edge codes added (the Tier-2 codebook unparked and switched from 2-bit to two-letter form; `/validate-idea` keep-with-modification); 2026-07-09 Polarity note added to Tier 2 section + [edge-sign](./edge-sign.md) links (candidate variant); 2026-04-30 original.
...
**The pattern is GoF Decorator**, adopted 2026-09-05 at [software-design-principles-for-neural-os](./software-design-principles-for-neural-os.md) §GoF Decorator. The unmarked edge is the Component; each modifier is a decorator that wraps it transparently, adds one responsibility, and can be withdrawn. Two things follow that §O alone does not give:

- **The cost of the alternative is a number.** Three independent modifiers without additivity means 2³ = **eight** named edge types just to cover presence and absence — a floor, since quantity alone has three renderings — doubling with each new modifier. That is what the rejected ladder below does not escape — a ladder orders the explosion rather than removing it.
- **No recursion, and that is the trade.** GoF lets you stack the same decorator twice (two borders); a mark here is a *position in a picture* and a picture position renders one value, so there is no second thickness for an already-thick stream. In exchange, Decorator's chief liability — a forest of look-alike wrappers — is structurally unreachable. Finite positions cost the feature and buy away the failure mode.
...
**Scope.** Codes serialize **a Tier 2 scene** — the four slot picks — and they do not encode or retrieve one. Use them for writing edges down — notes, diagrams, tooling, drill answers — never as the thing you rehearse. **Tier 1 edges never get a code**: an open verb needs no codebook, and giving it one is the ceremony [software-design-principles-for-neural-os](./software-design-principles-for-neural-os.md) §The Main Constraint rejects.

**Open — the code does not carry two of the three modifiers.** The sentence above used to read *"codes serialize an edge,"* which is true of a bare Tier 2 scene and false of a marked one. Of the three modifiers in §Modifier composition above, only [edge-sign](./edge-sign.md) has a written form (the suffix two paragraphs above). [Quantity](./encoding-quantities-in-cast.md) and [delay](./delay-encoding-in-cast.md) have none — not here, not on their own pages; `Q0·Q1·Q2` and `D0·D1·D2` name *which move you used*, not what the edge carries. So the worst-edge example this page holds up — **thick stream, long gap, interceptor** — writes as `Gi-Ex-Cl-Bl−`, character-identical to a thin, instant, inhibitory edge, and a diff over codes reports **no change** across exactly the contrast the modifiers exist to make visible.

Surfaced 2026-09-05 by adopting GoF Decorator for the lattice: GoF's chief drawback is that *a decorated component is not identical to the component, so do not rely on object identity* — and the machine view above is an identity built from the ConcreteComponent alone. **Named as a gap, not designed**: a written form is vocabulary, and the operator picks vocabulary. One constraint on it is already fixed, though, and it comes from the pattern rather than from taste — any modifier notation must be **self-delimiting**, readable without knowing in advance which modifiers are present, because any subset is legal. That is the written analogue of the property this section already prizes for slots ("the code decodes without knowing which slot you are in"). The sign suffix has it; a bare appended number would not.

**Failure mode — the `Me`/`Ma` pair.** Mermaid and Mage are the one within-slot near-pair (same letter, different vowel), and they are why single letters fail here at all. Cross-slot near-pairs (`Ro`/`Re`, `Gi`/`Gr`, `Cr`/`Cl`) are separated by position as well as by vowel; `Me`/`Ma` has only the vowel. Drill it as a discrimination pair or the collision the code was built to survive returns inside the code. The instrument exists since 2026-09-18: `python3 tools/cast_tier2_flash.py --report` prints a confusion table (expected → answered), so a `mermaid → mage` row is a count rather than a suspicion — see [cast-drill-ladder](./cast-drill-ladder.md) §Stage 4.
...
- [adjacency-is-not-an-edge](./adjacency-is-not-an-edge.md) — why placement alone is not enough: the three properties of an edge (direction · reach · type) that spatial adjacency drops, and why §Layer 2's edges-at-the-source rule repairs all three at once. Argues the routing table above
