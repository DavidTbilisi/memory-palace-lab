...
**Neither band is the cultural sequence you already know**, and this is where the confident wrong answers actually come from. [cultural-string-sequences](./cultural-string-sequences.md) §1 registers the rainbow as **ROYGBIV, seven**; band 8 runs to **ten**, completing the spectrum with three achromatics. §4 registers the planets as **Mercury→Neptune, eight, Pluto excluded post-2006**; band 9 runs to **ten**, taking the **Sun at 0** and **keeping Pluto at 9**, because a peg band must fill exactly ten slots. Both constructs are correct for their own job — a cultural sequence answers *what comes next*, a band supplies an address — but substituting one for the other leaves you **off by one and short by two**, with a confident answer either way. The carrier for the twenty cells and this trap is [sem3-ordinal-song](./sem3-ordinal-song.md).
...
**The one argued exception.** [sem3-linking-chains](./sem3-linking-chains.md) carries all eighty phonetic cells as eight causal story-chains, and it is admitted here because it does not reproduce the defect this rule guards against: **a chain cannot be drilled cell by cell** — you cannot reach item 4 without running the story — so the form forbids the per-cell rote the lapse data measures. What it installs is the residue the rule cannot generate: the consonant is fixed, but *which* S-word is the visual one is not.
...
- [sem3-song](./sem3-song.md) — the carrier song over this page; the ten categories as a fixed chorus, the rule and its two exceptions as story
- [sem3-linking-chains](./sem3-linking-chains.md) — eight causal story-chains, one per phonetic band, carrying the eighty cells this page declines to tabulate
- [sem3-chain-ballad](./sem3-chain-ballad.md) — those chains sung, one tune for all eight verses because the consonant ladder is identical down every band
- [sem3-love-poem](./sem3-love-poem.md) — the whole of this page carried as a love poem about loss; the lyric carrier beside the two drill carriers
- [sem3-ordinal-song](./sem3-ordinal-song.md) — the other half of that cycle; the twenty ordinal cells of bands 8 and 9, each sung with its digit, because the rule cannot reach them
- [cultural-string-sequences](./cultural-string-sequences.md) — the registry of the rainbow and planet sequences bands 8 and 9 extend and must not be confused with
