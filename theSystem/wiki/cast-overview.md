...
- [adjacency-is-not-an-edge](./adjacency-is-not-an-edge.md) — the argument for §Two-layer architecture's key rule: spatial adjacency drops direction, reach and type, and an edge living on its source node restores all three, decoupling graph topology from palace topology
